...
from nim_cli.core.streaming import StreamProcessor, StreamChunk
...
class NIMClient:
    ...
    async def chat(
        ...
        *,
        ...
        # Track request
        request_tracker = self._metrics.track_request(model)
        request_tracker.__enter__()
        ...
        try:
            ...
            return ChatResponse(
                ...
                latency_ms=0,  # Will be calculated by metrics
            )
            
        except Exception as e:
            request_tracker.__exit__(type(e), e, e.__traceback__)
            raise
        
        finally:
            request_tracker.__exit__(None, None, None)
        ...
        *,
        ...
        try:
            ...
            async for chunk in stream:
                ...
                async for processed in processor.process(self._bytes_iterator(chunk_bytes)):
                    yield processed
    ...
    async def _bytes_iterator(self, data: bytes) -> AsyncIterator[bytes]:
        """Create an async iterator for a single bytes object."""
        yield data
    
    def _chunk_to_bytes(self, chunk: ChatCompletionChunk) -> bytes:
        """Convert a chunk to SSE bytes format."""
        import json
