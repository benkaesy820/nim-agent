    ...
    def _config_to_dict(self, config: Config) -> dict[str, Any]:
        ...
        def convert_value(v: Any) -> Any:
            if isinstance(v, SecretStr):
                ...
                # model_dump() already returns plain types — no need to recurse
                return v.model_dump()
        ...
        result = {k: convert_value(v) for k, v in config.model_dump().items()}
        # Never write the API key to disk — use NVIDIA_API_KEY env var instead
