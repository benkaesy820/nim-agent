...
from nim_cli.core.config import Config, ConfigManager, get_config
    ...
    async def _handle_command(self, result: InputResult) -> bool:
        ...
        if cmd in ("exit", "quit", "q"):
            ...
            if args:
                ...
                self._console.print(f"Model: {self._config.model.name}")
            else:
            ...
            if args:
                try:
                    set_theme(args[0])
                    self._theme = get_theme()
                    self._console.print(f"Theme: {args[0]}")
        ...
        try:
            async with self.client as client:
                ...
                async for chunk in client.chat_stream(
                    ...
                    history=self._conversation.get_context()[:-1],  # Exclude current
                ):
