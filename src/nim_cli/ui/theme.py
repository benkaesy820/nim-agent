...
class Theme:
    ...
    def _init_styles(self) -> None:
        ...
        # Text styles
        self.text = Style(color=c.text)
        self.text_muted = Style(color=c.text_muted)
        self.text_highlight = Style(color=c.text_highlight, bold=True)
        
        # Semantic styles
        self.success = Style(color=c.success)
        self.warning = Style(color=c.warning)
        self.error = Style(color=c.error, bold=True)
        self.info = Style(color=c.info)
        
        # Message styles
        self.user_style = Style(
            color=c.user_message,
            bold=True,
        )
        self.assistant_style = Style(
            color=c.assistant_message,
        )
        self.system_style = Style(
            color=c.system_message,
            italic=True,
        )
        ...
        # Border colors (for Panel border_style parameter)
